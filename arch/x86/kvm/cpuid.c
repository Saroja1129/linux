...
//Saroja Submission CMPE 283
int kvm_emulate_cpuid(struct kvm_vcpu *vcpu)
{
	u32 eax, ebx, ecx, edx;
	extern u32 number_of_exits;
	extern u64 total_time_in_exits;
	extern u32 vmm_number_of_exits;
	extern u64 vmm_total_time_in_exits;
	...
	if(eax == 0x4FFFFFFF){ 
		eax = number_of_exits; 
		printk(KERN_INFO "For Input 0x4FFFFFFF, Output is %u", number_of_exitss);
	}
    //Assignment 3
		...
		if(ecx == 3 || ecx == 4 || ecx == 6 || ecx == 11 || ecx == 16 || ecx == 17 || ecx == 51 || ecx == 63 || ecx == 64 || ecx == 66 || ecx == 67 || ecx == 68 || ecx == 69){
			...
			ecx = 0;
			edx = 0xFFFFFFF;
			printk(KERN_INFO "CPUID(0x4ffffffd): Instruction Exit Type %u is not defined", (int) exit_type);
		}
		else {
			eax = (vmm_total_time_in_exits[ecx])>>32;
			ecx = (vmm_total_time_in_exits[ecx]) & 0xFFFFFFF;
			printk(KERN_INFO "CPUID(0x4ffffffc): Time elapsed for exit type %u is %llu cycles", (int) exit_type, vmm_total_time_in_exits[exit_type]);
			for (int i = 0; i < 70 ;i++) {
				printk(KERN_INFO "Time Elapsed for exit type %u is %llu cycles", i, vmm_total_time_in_exits[index]);
			}
    ...
    //Assignment 3
		...
		ebx = (unsigned long) total_time_in_exits >> 32;
		ecx = (unsigned long) total_time_in_exits & 0xffffffff;	
		printk(KERN_INFO "For Input 0x4FFFFFFE, total time elapsed is %llu", total_time_in_exits); 
	}
	else{
