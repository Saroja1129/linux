...
u32 count_of_exits = 0;
EXPORT_SYMBOL(count_of_exits); //Exporting so it can be used by cpuid.c

u32 exit_time; //This will store total time elapsed for various exits
EXPORT_SYMBOL(exit_time); //Exporting so it can be used by other modules

u32 vm_count_of_exits[70];
EXPORT_SYMBOL(vm_count_of_exits);

u32 vm_exit_type[70];
EXPORT_SYMBOL(vm_exit_type); 
...
{
	u64 exit_end_time = 0; //This will store the end of each exit
	u64 exit_start_time = get_current_time_stamp(); //to capture Start of the exit
	struct vcpu_vmx *vmx = to_vmx(vcpu);
	union vmx_exit_reason exit_reason = vmx->exit_reason;
	u32 vectoring_info = vmx->idt_vectoring_info;
	u16 exit_handler_index;
	count_of_exits++; //Incrementing the exit counter to be used in cpuid.c
	...
	*/  
	...
	vm_count_of_exits[exit_reason.basic]++;
	/*
	...
	if (KVM_BUG_ON(vmx->nested.nested_run_pending, vcpu->kvm)){
		...
		*/
		...
		exit_end_time = get_current_time_stamp();
		exit_time += exit_end_time - exit_start_time;
		vm_exit_type[exit_reason.basic] += exit_time;
		return -EIO;
